...
export default function ArticlePage() {
  ...
  const authorImage = article.auteur?.imageUrl || article.author?.image?.asset?.url || article.author?.imageUrl;
  const authorRole = article.author?.role;
  const authorBio = article.auteur?.bio || article.author?.bio;
  const authorSpecialites = article.author?.specialites;
  const authorSocial = article.author?.social || {
    linkedin: article.author?.linkedin || article.auteur?.linkedin,
    instagram: article.author?.instagram || article.auteur?.instagram,
    twitter: article.author?.twitter || article.auteur?.twitter
  };
